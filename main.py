...
for clf_tgt in iteration['clf_targets']:
    ...
    print("pat_fram_train_clf_norm upsampled %s to %d" % (over_samp_names[o_s], pat_frame_train_clf_norms[o_s].shape[0]))
    # pat_test
    ...
    for n in iteration['n']: # standard, minmax, (robust-quantile-based) normalizations of input data
        ...
        # compute t_feats
        t_frame = t_frame_compute(pat_frame_train_reg, con_frame, ['thickness', 'area', 'volume'])
    ...
    brfc_predictions = brfc_best5_models_with_feats[0].predict(
        pat_frame_test_reg_norms[n].loc[:, brfc_best5_models_with_feats[1]])

    brfc_score = brfc_best5_models_with_feats[0].score(pat_frame_test_reg_norms[n].loc[:, brfc_best5_models_with_feats[1]],
    ...
    exp_description = '**balRandTest'+str(t_s)+'_RegTrainRest_ClfTrain' + over_samp_names[o_s] + '_' + norm + '_' \
                      ...
                      '**t_allRegTrain_DesikanThickAreaVolFeats_TorP'
